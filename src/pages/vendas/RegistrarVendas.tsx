...
// ==========================
// AJUSTES MÍNIMOS para evitar voltar 1 mês (timezone)
// ==========================
// 1) Monta Date a partir de "YYYY-MM-01" usando construtor numérico (local), sem UTC
const parseYYYYMMToLocalDate = (yyyyMmOrDateISO: string): Date => {
  // espera algo como "2024-04-01" (ou ISO iniciado por isso)
  const parts = yyyyMmOrDateISO.slice(0, 10).split("-");
  const y = Number(parts[0]);
  const m = Number(parts[1]);
  const d = Number(parts[2] || 1);
  return new Date(y, (m || 1) - 1, d || 1, 12, 0, 0, 0); // meio-dia local evita divergências
};
...
interface Venda {
  id: number;
  data: string; // "YYYY-MM-01" (date no Postgres) ou ISO começando com isso
...
export default function RegistrarVendas() {
  ...
  async function fetchVendedoras() {
    ...
    if (error) toast({ title: "Erro ao carregar vendedoras", variant: "destructive" });
  ...
  async function fetchFiliais() {
    ...
    if (error) toast({ title: "Erro ao carregar filiais", variant: "destructive" });
    else setFiliais(data || []);
  }

  async function fetchVendas() {
    const { data, error } = await supabase
      .from("vendas_diarias")
      .select(`*, pessoas_fisicas(nome_completo), filiais(nome)`) // mantém selection original
  ...
  async function onSubmit(values: VendaFormData) {
    setLoading(true);
    try {
      // Mantém a sua lógica de montar string YYYY-MM-01 (o banco grava como date sem timezone)
      const mesFormatado = values.mes.toString().padStart(2, "0");
      const dataString = `${values.ano}-${mesFormatado}-01`;

      const vendaData = {
        data: dataString, // **string YYYY-MM-01** (sem Date.parse, evita UTC)
      ...
      toast({ title: editingId ? "Venda atualizada!" : "Venda registrada!", description: "Dados salvos com sucesso." });
      ...
      toast({ title: "Erro ao salvar venda", description: error instanceof Error ? error.message : "Erro desconhecido", variant: "destructive" });
    } finally {
      setLoading(false);
    }
  }
  ...
  function editVenda(venda: Venda) {
    ...
    // **AQUI o ajuste crucial**: parse local usando construtor numérico para não cair no mês anterior
    const vendaData = parseYYYYMMToLocalDate(venda.data);
    form.reset({
  ...
  async function deleteVenda(id: number) {
    ...
    if (error) toast({ title: "Erro ao excluir venda", variant: "destructive" });
    else { toast({ title: "Venda excluída com sucesso" }); fetchVendas(); }
  }
  ...
  async function exportToCSV() {
    const csv = [
      ...
      ...vendas.map((v) => {
        const d = parseYYYYMMToLocalDate(v.data);
        return [
          format(d, "dd/MM/yyyy"),
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Cadastre vendas do mês por vendedora ou importe via CSV/Excel</p>
      </div>

      <Tabs defaultValue="manual" className="w-full">
        <TabsList>
          <TabsTrigger value="manual"><Plus className="mr-2 h-4 w-4" />Cadastro Manual</TabsTrigger>
          <TabsTrigger value="importar"><Upload className="mr-2 h-4 w-4" />Importar CSV/Excel</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="manual">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Registre as vendas totais do mês para cada vendedora</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <FormField control={form.control} name="mes" render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={(val) => field.onChange(parseInt(val))} value={field.value?.toString()}>
                          ...
                          <SelectContent>
                            {[{value:1,label:"Janeiro"},{value:2,label:"Fevereiro"},{value:3,label:"Março"},{value:4,label:"Abril"},{value:5,label:"Maio"},{value:6,label:"Junho"},{value:7,label:"Julho"},{value:8,label:"Agosto"},{value:9,label:"Setembro"},{value:10,label:"Outubro"},{value:11,label:"Novembro"},{value:12,label:"Dezembro"}].map(m => (
                    ...
                    <FormField control={form.control} name="vendedora_pf_id" render={({ field }) => (
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent>
                            {vendedoras.map(v => (<SelectItem key={v.id} value={v.id.toString()}>{v.nome_completo}</SelectItem>))}
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                    ...
                    <FormField control={form.control} name="filial_id" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="valor_bruto_centavos" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="desconto_centavos" render={({ field }) => (
                      <FormItem>
                    ...
                    <FormField control={form.control} name="qtd_itens" render={({ field }) => (
                      <FormItem>
                  ...
                  <div className="bg-muted p-4 rounded-lg">
                    <p className="text-sm font-medium">Valor Líquido do Mês: <span className="text-lg">{formatCurrency(valorLiquido)}</span></p>
                  </div>

                  <div className="flex gap-2">
                    <Button type="submit" disabled={loading}>
                      {loading ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" />Salvando...</>) : (<><Plus className="mr-2 h-4 w-4" />{editingId ? "Atualizar Venda" : "Registrar Venda"}</>)}
                    </Button>
                    {editingId && (
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {vendas.map((venda) => {
                const d = parseYYYYMMToLocalDate(venda.data); // **sem UTC**
                  ...
                  <TableRow key={venda.id}>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button variant="ghost" size="icon" onClick={() => editVenda(venda)}><Edit className="h-4 w-4" /></Button>
                        <Button variant="ghost" size="icon" onClick={() => deleteVenda(venda.id)}><Trash2 className="h-4 w-4" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                );
