...
export function ContasRecorrentes() {
  ...
  const loadData = async () => {
    setLoading(true);
    await Promise.all([fetchContas(), fetchFornecedores(), fetchFornecedoresPF(), fetchCategorias(), fetchFiliais()]);
    setLoading(false);
  };

  const fetchContas = async () => {
    const { data, error } = await supabase.from("contas_recorrentes").select("*").order("descricao");
  ...
  const fetchFornecedoresPF = async () => {
    const { data } = await supabase.from("pessoas_fisicas").select("id, nome").order("nome");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const dataToSubmit = {
      ...
      fornecedor_id:
        formData.tipo_fornecedor === "pj" && formData.fornecedor_id ? parseInt(formData.fornecedor_id) : null,
      pessoa_fisica_id:
        formData.tipo_fornecedor === "pf" && formData.pessoa_fisica_id ? parseInt(formData.pessoa_fisica_id) : null,
    ...
    try {
      if (editingConta) {
        const { error } = await supabase.from("contas_recorrentes").update(dataToSubmit).eq("id", editingConta.id);
        if (error) throw error;
        toast({ title: "Sucesso", description: "Conta recorrente atualizada." });
      } else {
        const { error } = await supabase.from("contas_recorrentes").insert([dataToSubmit]);
        if (error) throw error;
  ...
  const gerarContasMes = async () => {
    ...
    for (const conta of ativas) {
      ...
      const { data: existe } = await supabase.from("contas_pagar").select("id").eq("descricao", descricaoMes).limit(1);
  ...
  const filteredContas = contas.filter((c) => c.descricao?.toLowerCase().includes(searchTerm.toLowerCase()));
              ...
              {filteredContas.length === 0 ? (
                ...
                filteredContas.map((conta) => (
                  <TableRow key={conta.id}>
                    ...
                    <TableCell>{conta.dia_vencimento || 1}</TableCell>
                    <TableCell className="text-right">
                      R$ {((conta.valor_centavos || 0) / 100).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
                    </TableCell>
                    <TableCell>
                      <Badge variant={conta.ativo ? "default" : "secondary"}>{conta.ativo ? "Ativa" : "Inativa"}</Badge>
                    </TableCell>
                    <TableCell className="text-right">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setFormData({ ...formData, dia_vencimento: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v: "pj" | "pf") =>
                    setFormData({ ...formData, tipo_fornecedor: v, fornecedor_id: "", pessoa_fisica_id: "" })
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
              ...
              <div>
                ...
                {formData.tipo_fornecedor === "pj" ? (
                  ...
                  <Select
                    value={formData.pessoa_fisica_id}
                    onValueChange={(v) => setFormData({ ...formData, pessoa_fisica_id: v })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Selecione..." />
                    </SelectTrigger>
