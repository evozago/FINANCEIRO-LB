...
export interface DadosEtiqueta {
  ...
  rem_cnpj?: string;
  // Dados do envio
  awb?: string;  // opcional: pode não estar disponível para etiquetas geradas pelo portal
  rota?: string;
  pedido: string;
  volumes?: number;
  peso_kg?: number;
...
export function gerarEtiquetaPDF(dados: DadosEtiqueta): void {
  try {
    ...
    // ===== CÓDIGO DE BARRAS (só se tiver AWB) =====
    if (awb) {
      const barcodeImg = gerarCodigoBarrasSVG(awb);
      if (barcodeImg) {
      ...
      doc.setFontSize(6.5);
      doc.text(awb, margin + 90, margin + 80, { angle: 90 });
    } else {
      // Sem AWB: exibir aviso
      doc.setFont('helvetica', 'italic');
      doc.setFontSize(8);
      doc.setTextColor(150, 150, 150);
      doc.text('AWB não disponível', margin + 2, margin + 72);
    ...
    alert('Erro ao gerar etiqueta. Verifique o console para mais detalhes.');
  }
}
